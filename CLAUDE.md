...
Personal website (sebastianwie.land) — an Angular 13 SPA, pre-rendered to static HTML with Scully and deployed to GitHub Pages. Its main feature is a contact form that PGP-encrypts a full MIME message **in the browser** before posting it to an API, plus a page that verifies PGP signatures on `.eml` files exported from mail clients.

## Commands

```bash
npm start                    # ng serve --ssl (HTTPS; dev backend is https://localhost:8001/api)
npm run build                # production build → dist/sebastianwie-land
npm test                     # Karma/Jasmine, watch mode, Chromium browser
npx scully --scanRoutes      # pre-render → dist/static (run AFTER npm run build)
npm run scully:serve         # serve the pre-rendered output
```

Run a single spec: `npx ng test --include='**/encoding.service.spec.ts'`. Karma's configured browser is `Chromium` (not `Chrome`) — override with `--browsers ChromeHeadless` in CI-like runs.

There is no linter configured.

CI (`.github/workflows/publish.yml`) runs build → scully → `peaceiris/actions-gh-pages` publishing `dist/static` on every push to `master`. Scully's puppeteer is pinned to `/usr/bin/chromium-browser`; if that path changes, `scully.sebastianwie-land.config.ts` must change too.

## Architecture

### Routing & modules
`AppRoutingModule` lazy-loads two feature modules: `sites/*` → `ContentModule`, `contact/*` → `ContactModule`. Legacy `/signature-info` and `/signature-info.html` redirect to `contact/verify` and are listed in `extraRoutes` in the Scully config so they get pre-rendered — **any new route not discoverable by `--scanRoutes` must be added there**, or it 404s on GitHub Pages.

Content pages are markdown-driven: `ContentRoutingModule` maps a route to `MarkdownComponent` via `data.markdownFile` pointing at `src/assets/content/*.md` (rendered by `ngx-markdown`). Adding a prose page = add a `.md` file plus a route entry, no new component.
...
3. `OpenpgpService.encrypt` encrypts the whole document with the site's public key.
...
### Environments
`src/environments/environment.ts` (dev) / `.prod.ts` hold `backendUrl` and the `contact` config. Email addresses are stored as char-code arrays and reassembled at runtime to keep them out of GitHub scrapers — keep that obfuscation when editing them.

### Static assets shipped verbatim
`src/gh-pages/` is copied to the build root (`CNAME`, `.nojekyll`, `keybase.txt`, `pgp-pubkey.asc`). Rotating the PGP key means replacing `pgp-pubkey.asc` there.
...
- Component selector prefix `nwie`; SCSS everywhere; TypeScript strict mode.
